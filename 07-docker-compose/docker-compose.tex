...
\begin{itemize}
\item Статический веб-сайт работает в nginx контейнере
\item Развертывается автоматически через CI/CD pipeline
\item Изолирован от других приложений на сервере
\end{itemize}
...
\begin{itemize}
\item \textbf{1 nginx контейнер} — веб-сервер и reverse proxy
...
\textbf{Задача:} как эффективно управлять всеми этими контейнерами?
\end{frame}
...
docker run -d --rm --name backend-1 busybox:stable sleep 250
docker run -d --rm --name backend-2 busybox:stable sleep 250
docker run -d --rm --name backend-3 busybox:stable sleep 250
docker run -d --network demo-net -p 8080:80 --name nginx nginx:alpine
\end{verbatim}
...
\item \textbf{Изоляция сетей} — автоматическое создание внутренних сетей
\end{itemize}
...
\begin{itemize}
  ...
  \item \href{https://docs.docker.com/compose/compose-file/07-volumes/}{Тома} — постоянное хранение данных
...
\begin{itemize}
  \item \textbf{Docker Swarm} — простая альтернатива Kubernetes от Docker
