\documentclass[aspectratio=169]{beamer}

\input{../slides-common}

\title{Packer}
\date{22 апреля 2026 г. }

\begin{document}
    \maketitle

    \begin{frame}{Два подхода к управлению конфигурацией}
        \begin{columns}[t]
            \begin{column}[t]{0.48\textwidth}
                \textbf{Baked (образы):}
                \begin{itemize}
                    \item Заранее готовим образ с настроенным ПО
                    \item Разворачиваем готовые серверы из образа
                    \item Быстрый запуск
                \end{itemize}
                
                \vspace{0.3cm}
                \textbf{Примеры:} Docker, Packer и др.
            \end{column}
            
            \begin{column}[t]{0.48\textwidth}
                \textbf{Fry (конфигурация):}
                \begin{itemize}
                    \item Берем чистый сервер
                    \item Автоматически настраиваем его скриптами
                    \item Гибкость в настройке
                \end{itemize}
                
                \vspace{0.3cm}
                \textbf{Примеры:} Ansible, Chef, и др.
            \end{column}
        \end{columns}
        
    \end{frame}

    \begin{frame}{Baked vs Fry}
        \textbf{Плюсы baked:}
        \vspace{0.2cm}
        \begin{itemize}
            \item Предсказуемость и идентичность окружений
            \item Отсутствие configuration drift
            \item Быстрое масштабирование
        \end{itemize}

        \vspace{0.3cm}
        \textbf{Плюсы fry:}
        \vspace{0.2cm}
        \begin{itemize}
            \item Проще в настройке и поддержке
            \item Гибкость и скорость изменений
            \item Применимо к большинству приложений и серверов (в том числе к bare metal серверам)
        \end{itemize}
    \end{frame}

    \begin{frame}{Как реализуется baked подход?}

        \textbf{Образ} — это снимок (копия, шаблон) готовой системы со всеми настройками и приложениями

        \textbf{Применяется для:}
        \begin{itemize}
        \item \textbf{Виртуализация}: создание образов виртуальных машин (\href{https://en.wikipedia.org/wiki/Virtual_appliance}{Virtual appliance})
        \item \textbf{Контейнеризация}: создание образов контейнеров
        \end{itemize}

        \textbf{Процесс создания образа:}
        \begin{itemize}
            \item Развертывание базового образа
            \item Настройка системы и установка ПО
            \item Сохранение итогового образа
        \end{itemize}

        \footnotesize{\textcolor{gray}{\url{https://en.wikipedia.org/wiki/Software_appliance\#Types_of_software_appliances}}}
    \end{frame}

    \begin{frame}{Демонстрация: Создание VA в VirtualBox}
        \textbf{Что посмотрим:}
        \begin{itemize}
            \item Ручная установка Alpine Linux в VirtualBox
            \item Ручная настройка системы, установка nginx
            \item Создание образа
        \end{itemize}
    \end{frame}

    \begin{frame}{Виды Virtual Appliance}
        \begin{itemize}
            \item VA общего назначения: \href{https://www.osboxes.org}{OSBoxes}, \href{https://portal.cloud.hashicorp.com/vagrant/discover}{Vagrant Boxes}, \href{https://cloud-images.ubuntu.com/}{Ubuntu Cloud Image}
            \item Специализированные VA: \href{https://www.turnkeylinux.org/}{TURNKEY images}, корпоративные VA
            \item \href{https://images.linuxcontainers.org/images}{LXC контейнеры}
        \end{itemize}
    \end{frame}

    \begin{frame}{Что запекать в Image?}
        \begin{columns}[t]
            \begin{column}[t]{0.465\textwidth}
                \textbf{Запекаем в Image:}
                \begin{itemize}
                    \item ОС и системные зависимости
                    \item Зависимости приложений
                    \item Приложение
                    \item Дефолтные конфиги приложения
                \end{itemize}
            \end{column}

            \begin{column}[t]{0.465\textwidth}
                \textbf{НЕ запекаем в Image:}
                \begin{itemize}
                    \item Секреты
                    \item Окружение-зависимая конфигурация
                    \item Build зависимости
                \end{itemize}
            \end{column}
        \end{columns}
    \end{frame}

    \begin{frame}{Что такое Packer?}
        \textbf{Packer} — инструмент для автоматизированного создания образов виртуальных машин
        
        \vspace{0.5cm}
        
        \textbf{Основные возможности:}
        \begin{itemize}
            \item Автоматизация создания образов виртуальных машин
            \item Поддержка разных платформ (VirtualBox, VMware, облачные провайдеры)
            \item Разные способы настройки (Shell, Ansible и \href{https://developer.hashicorp.com/packer/integrations}{другие})
        \end{itemize}
    \end{frame}

    \begin{frame}{Архитектура Packer}
        \textbf{Процесс создания образа:}
        
        \begin{enumerate}
            \item Packer запускает временную VM
            \item Подключается к ней по SSH
            \item Выполняет скрипты установки и настройки
            \item Создает образ из настроенной VM
            \item Удаляет временную VM
        \end{enumerate}
        
        \vspace{0.5cm}
        
        \textbf{Компоненты Packer:}
        \begin{itemize}
            \item \textbf{Builder} — создает VM (virtualbox, vmware, облачные)
            \item \textbf{Provisioner} — настраивает VM (shell, ansible)
            \item \textbf{Template} — HCL файл с описанием процесса
        \end{itemize}
    \end{frame}


    \begin{frame}{Демонстрация: Packer + VirtualBox}
        \textbf{Что посмотрим:}
        \begin{itemize}
            \item Packer template для VirtualBox
            \item Автоматизация процесса создания образов
        \end{itemize}

        \vspace{0.2cm}
        \textbf{Демо-репозиторий:} \\
        \href{https://github.com/prafdin/devops-demo-website/tree/new-packer-demo}{\texttt{https://github.com/prafdin/devops-demo-website/tree/new-packer-demo}}
    \end{frame}
\end{document}